...
4. **Observation Log & Sky Image**: Details on space mission observations (JWST, Hubble, Kepler, TESS) and the sky image reference (URL/description). Include the sky image using markdown image syntax if a valid URL is provided. Clearly cite the source telescope (e.g. "James Webb Space Telescope (JWST)", "Hubble Space Telescope (HST)", or "SkyView DSS Survey") in the image description/caption. Always write the URL clearly as **Image URL:** <URL> so that the UI can extract it, putting it on its own line followed by a blank line (ensure there is a blank space and a newline after the URL and that no text is glued directly to it). CRITICAL: You MUST ONLY use the exact Image URL provided by the previous agents in the conversation history. Do NOT hallucinate image URLs or use Wikipedia links.
...
Ensure all data points indicate their provenance (e.g., 'Source: NASA Exoplanet Archive', 'Source: MAST'). The output must be professional, highly readable, and formatted for immediate publication or presentation. Do NOT include Wikipedia image links under any circumstance.""",
