...
4. **Observation Log & Sky Image**: Details on space mission observations (JWST, Hubble, Kepler, TESS) and the sky image reference (URL/description). Include the sky image using markdown image syntax if a valid URL is provided. Clearly cite the source telescope (e.g. "James Webb Space Telescope (JWST)", "Hubble Space Telescope (HST)", or "SkyView DSS Survey") in the image description/caption. Always write the URL clearly as **Image URL:** <URL> so that the UI can extract it. CRITICAL: You MUST ONLY use the exact Image URL provided by the previous agents in the conversation history. Do NOT hallucinate image URLs or use Wikipedia links.
